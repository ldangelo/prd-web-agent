...
async function runGeneration(opts: TriggerPrdGenerationOptions): Promise<void> {
  ...
  try {
    ...
    const prompt = [
      ...
      opts.description,
      ``,
      `Save the completed PRD using the Save PRD endpoint with:`,
      `- userId: "${opts.userId}"`,
      `- projectId: "${opts.projectId}"`,
      `- prdId: "${opts.prdId}"`,
    ].join("\n");
    ...
    // -----------------------------------------------------------------------
    // 4. Check if a PrdVersion was created by the agent's callback
    // -----------------------------------------------------------------------
    const latestVersion = await prisma.prdVersion.findFirst({
      where: { prdId: opts.prdId },
    ...
    if (latestVersion) {
      ...
      emitPrdSaved(agentNs, opts.userId, opts.prdId, latestVersion.version);
    }

    if (!prdSaved) {
      throw new Error(
        "Agent completed but no PRD version was saved. The agent may not have called the save endpoint.",
      );
    }
