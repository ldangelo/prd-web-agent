...
**Use `appUrl` and `internalToken` from the session context** — do NOT use `{{APP_URL}}` or `{{OPENCLAW_INTERNAL_TOKEN}}` placeholders.

### Save PRD

POST {appUrl}/api/internal/prd/save
...
GET {appUrl}/api/internal/prd/read?identifier=...&userId=...
Authorization: Bearer {internalToken}

### List PRDs

GET {appUrl}/api/internal/prd/list?userId=...&projectId=...&search=...
Authorization: Bearer {internalToken}
